...
async def test_list_todos_filter_title_should_return_5_todos(
    ...
    response = client.get(
        "/todos/?tittle=Test todo 1", headers={"Authorization": f"Bearer {token}"}
    )
    assert len(response.json()["todos"]) == expected_todos
...
async def test_list_todos_filter_description_should_return_5_todos(
    ...
    await session.commit()
    response = client.get(
        "/todos/?desciption=desc", headers={"Authorization": f"bearer {token}"}
    )
    assert len(response.json()["todos"]) == expected_todos
...
async def test_delete_other_user_todo(session, client, token, other_user):
    ...
    session.add_all([todo_other_user])
    await session.commit()

    response = client.delete(
        f"/todos/{todo_other_user.id}", headers={"Authorization": f"Bearer {token}"}
    )
